"""Video tools for PART 7. All ffmpeg calls go through imageio-ffmpeg, so no separate install is needed.
...
def read_sources():
    """assets/SOURCES.txt -> {cut_number: [lines]}. A cut may have one line or several:
         cut3 | Kling | prompt... | 2026-09-10
         cut3 | credit | "Video by NAME from Pixabay", shown in the final cut
         cut3 | license | CC BY 4.0
    Every line starts with cutN and has at least two " | " separated fields."""
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if len(parts) >= 2 and parts[0].lower().startswith("cut") and parts[0][3:].isdigit():
            out.setdefault(int(parts[0][3:]), []).append(line.strip())
    return out
